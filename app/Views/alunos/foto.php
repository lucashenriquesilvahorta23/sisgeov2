        ...
        <div class="row">		
            <div class="col-12">         
                <div class="box">
                    ...
                    <h3 class="box-title">Anexos - <?= $aluno->NOME_ALUNO; ?></h3>
                    <div class="box-controls pull-right">
                        <!-- <button id="row-remove" class="btn btn-xs btn-danger">Delete selected row</button> -->
                        <a href="/Aluno/FotoInserir/<?php echo $id_aluno; ?>"><button class="btn btn-md btn-info"><i class="fa fa-plus"></i> Novo</button></a>
                    </div>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                                    ...
                                    if($resultados != ""){
                                        foreach ($resultados as $resultado) {
                                            echo '<tr>';
                                                ...
                                                echo '		</a>';
                                                echo '		<a href="/Aluno/FotoEditar/'.base64_encode($resultado->ID_DOCUMENTO_ALUNO).'" >';
                                                echo '<button class="btn btn-success btn-list-icon" title="Editar informações"><i class="fa fa-edit "></i></button>';
                                                echo '		</a>';
                                                echo '<a href="'.base_url().'/Aluno/FotoExcluir/'.base64_encode($resultado->ID_DOCUMENTO_ALUNO).'/'.base64_encode($id_aluno).'" class="btn btn-danger btn-list-icon" title="Excluir informação" onclick="return confirm(\'Tem certeza que deseja apagar?\');"><i class="fa fa-trash"></i></a>';
                                                echo '	</td>';
